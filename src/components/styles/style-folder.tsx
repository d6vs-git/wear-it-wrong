...
//     const xSpacing = 60; // Horizontal spacing between images
//     const ySpacing = 30; // Vertical spacing between rows
...
const getFolderAnimations = (folderPath: string) => {
  ...
  const radius = 100; // distance from folder top center
  const startAngle = -160; // left-most angle
  const endAngle = -20; // right-most angle

  return Array.from({ length: count }, (_, i) => {
    const angle = startAngle + ((endAngle - startAngle) / (count - 1 || 1)) * i;
    const rad = (angle * Math.PI) / 180;

    const xOffset = Math.cos(rad) * radius + (Math.random() - 0.5) * 6;
    const yOffset = Math.sin(rad) * radius + (Math.random() - 0.5) * 3;
...
export default function StyleFolder({
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      <div
        className="relative w-64 h-48 cursor-pointer overflow-visible"
        onMouseEnter={handleHoverStart}
        onMouseLeave={handleHoverEnd}
        onClick={handleClick}
      >
        {/* Pop-up images above folder */}
        {animationData.map((imageData, index) => (
          <motion.div
            key={imageData.id}
            ...
            animate={
              isHovered
                ? {
                    x: imageData.hover.x,
                    y: imageData.hover.y - 24, // shift slightly above folder
                    rotate: imageData.hover.rotate,
                    scale: imageData.hover.scale,
                    opacity: 1,
                  }
